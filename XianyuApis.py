    ...
    def get_token(self, device_id, retry_count=0):
        if retry_count >= 2:  # 最多重试3次
            ...
            if self.hasLogin():
                ...
                raise RuntimeError("Cookie 已失效，请手动更新本地 .env 中的 COOKIES_STR 后重试。公开学习版不会自动写入配置。")
            ...
            if isinstance(res_json, dict):
                ...
                if not any('SUCCESS::调用成功' in ret for ret in ret_value):
                    ...
                    if 'RGV587_ERROR' in error_msg or '被挤爆啦' in error_msg:
                        ...
                        raise RuntimeError("触发平台风控或 Cookie 失效。请在本地手动更新合法的 COOKIES_STR 后重试，公开学习版不会交互式索取 Cookie。")
