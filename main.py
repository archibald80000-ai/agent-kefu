...
def configure_console_encoding():
    """强制 stdout/stderr 使用 UTF-8，避免 Windows 控制台乱码。"""
...
def validate_live_config():
    ...
    for key, invalid_values in placeholders.items():
        if is_placeholder(os.getenv(key), invalid_values):
            missing.append(key)
    return missing


def run_learning_mode():
    """公开学习版默认入口：不触发任何真实平台交互。"""
    logger.info("当前为学习模式：ENABLE_LIVE_PLATFORM=false。")
    logger.info("程序不会验证 Cookie、不会建立 WebSocket，也不会发起真实平台请求。")
    logger.info("如需在本地合法自测，请复制 .env.example 为 .env，填写你自己的配置，并将 ENABLE_LIVE_PLATFORM 改为 true。")
...
if __name__ == '__main__':
    ...
    enable_live_platform = env_flag("ENABLE_LIVE_PLATFORM", default=False)
    if not enable_live_platform:
        run_learning_mode()
        sys.exit(0)

    missing = validate_live_config()
    if missing:
        logger.error(f"真实接入模式缺少必要配置: {', '.join(missing)}")
        logger.error("公开学习版不会交互式索要或写入 Cookie，请在本地 .env 中手动填入你的合法配置后重试。")
        sys.exit(1)
